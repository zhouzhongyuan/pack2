...
function pack(cfg,winston){
    var o = new Object();
    o.build = function(){
        return co(function*(){
            winston.info("pack enviroment initializing......")
            yield o.preparePack();
            yield o.emptyDir('working');
            process.chdir('working');
            winston.info("pack enviroment initialize success")
            winston.info("create cordova begin")
            yield o.createCordova();
            winston.info("create cordova success")
            //TODO svn user
            winston.info("svn checkout app files begin")
            yield o.emptyDir(o.svnDir);
            yield o.getSvn(o.baseSvn,o.svnDir, 'zhouzy','zhouzy');
            winston.info("svn checkout app files success")
            winston.info("svn checkout project files begin")
            yield o.emptyDir(o.projectDir);
            yield o.getSvn(o.projectSvn, o.projectDir,  'zhouzy','zhouzy');
            winston.info("svn checkout project files success")
            yield o.processCode();
    ...
    o.processCode = function(){
        return new Promise(function (resolve, reject) {
            winston.info('parser config.xml');
            var conf = new ConfigParser(o.configXML);
            conf.setPackageName('忽悠');
            try {
    ...
    o.processCode = function(){
        ...
        return new Promise(function (resolve, reject) {
            ...
            //TODO preference
            conf.addElement('WebViewBounce',{'value':'false'});
            conf.addElement('DisallowOverscroll',{'value':'true'});

            conf.write();
            resolve('cogfig.xml ok');
        });
    };
    o.cleanPlatform = function (platform){
        return new Promise(function (resolve, reject) {
            var platform = platform || o.appPlatform ;
            winston.info('clean platform ',platform,' begin');
            cordova.clean(platform,{'verbose': true},function (err, data) {
                if (err) {
                    reject(new Error(err))
                }
                winston.info('clean platform ',platform,' success');
                resolve(data);
            });
        });
    };
    o.addPlatform = function (platform){
        return new Promise(function (resolve, reject) {
            var platform = platform || o.appPlatform ;
            winston.info('add platform', platform,'begin');
            cordova.platform('add', platform,{'verbose': true},function (err, data) {
                if (err) {
                    reject(new Error(err))
                }
                winston.info('add platform',platform,'success');
                resolve(data);
            });
        });
    };
    o.listPlatform = function(){
        winston.info('list platform begin, dir:',process.cwd());
        return new Promise(function (resolve, reject) {
            cordova.platform('ls', function (err, data) {
                if (err) {
                    reject(new Error(err))
                }
                winston.info('list platform','success');
                resolve(data);
            });
        });
    };
    ...
    o.preparePlatform = function(){
        return new Promise(function (resolve, reject) {
            winston.info('prepare platform begin');
    ...
    o.addPluginReal = function(plugin,variable){
        winston.info(plugin,variable);
        winston.info('begin to add plugin');
        return new Promise(function (resolve, reject) {
            ...
            cordova.plugin('ls',function(err,data){
                ...
                if( data.length != 0){
                    cordova.plugin('remove', data,function (err, data) {
                        ...
                        winston.info('plugin remove ','success');
                        resolve(data);
                    });
                }else {
                    resolve('no plugin');
    ...
    o.buildExtras = function(){
        return new Promise(function (resolve, reject) {
            winston.info('add build extras begin');
            var lintOptions =
                "android {\n" +
                "    lintOptions {\n" +
    ...
    o.buildApp = function(){
        return new Promise(function (resolve, reject) {
            winston.info('build',o.platform,'begin')
            ...
            cordova.build({platforms:[o.platform],options:{"release":buildType,"silent":false,"device":true}},function (err, data) {
                ...
                resolve(data);
                winston.info('build',o.platform,'success')
            });
        });
    };
    ...
    o.emptyDir = function(dir){
        winston.info('empty',dir,'begin');
        return new Promise(function (resolve, reject) {
            fs.emptydir(dir,function (err, data) {
                if (err) {
                    reject(new Error(err))
                }
                winston.info('empty',dir,'success');
                resolve(data);
            });
        });
    };
